...
class FOCINode(Node):
    ...
    def reset_robot(self):
        ...
        try:
            ...
            if not goal_done.wait(timeout=20.0):  # Increased from 10s to 20s
                self.get_logger().error('Reset goal send timeout')
                return False
            ...
            if not result_done.wait(timeout=25.0):  # Increased from 15s to 25s
                self.get_logger().error('Reset execution timeout')
                return False
        ...
        try:
            ...
            for i, pose_matrix in enumerate(poses):
                ...
                if not goal_done.wait(timeout=10.0):
                    self.get_logger().error(f'Goal {i} send timeout')
                    return {'status': 'failed', 'message': f'Goal {i} send timeout'}
                
                goal_handle = goal_handle_container[0]
                if not goal_handle or not goal_handle.accepted:
                    self.get_logger().error(f'Goal {i} rejected by MoveIt')
                ...
                if not result_done.wait(timeout=30.0):
                    self.get_logger().error(f'Waypoint {i} execution timeout')
                    return {'status': 'failed', 'message': f'Waypoint {i} execution timeout'}
                
                result = result_container[0]
                if not result or result.result.error_code.val != 1:  # 1 = SUCCESS
                    error_code = result.result.error_code.val if result else 'unknown'
                    self.get_logger().error(f'Execution failed at waypoint {i} (error: {error_code})')
